...
import { NodeId, NodeIdType } from '../../types/src/index.js';

// Re-export types package classes and enums for backward compatibility
export { NodeId, NodeIdType };
 ...
 * @see FR-012 - Compact encoding format selection
 */
export function encodeBinary(encoder: IEncoder, value: NodeId): void {
 ...
 * @see FR-013 - Support all six NodeId formats
 */
export function decodeBinary(decoder: IDecoder, maskBits: number = 0): NodeId {
...
/**
 * Register NodeId type with CodecFacade.
 * Note: Using 'i=17' as encoding ID per OPC UA spec.
 */
export function registerNodeId(facade: CodecFacade): void {
  facade.registerType('NodeId', 'i=17', EncodingFormat.Binary, encodeBinary, decodeBinary);
}

/**
 * Helper function to create a numeric NodeId.
 */
export function numericNodeId(namespace: number, identifier: number): NodeId {
  return new NodeId(namespace, identifier);
}

/**
 * Helper function to create a string NodeId.
 */
export function stringNodeId(namespace: number, identifier: string): NodeId {
  return new NodeId(namespace, identifier);
}

/**
 * Helper function to create a GUID NodeId.
 */
export function guidNodeId(namespace: number, identifier: string): NodeId {
  return new NodeId(namespace, identifier);
}

/**
 * Helper function to create a ByteString NodeId.
 */
export function byteStringNodeId(namespace: number, identifier: Buffer | Uint8Array): NodeId {
  return new NodeId(namespace, identifier);
}
