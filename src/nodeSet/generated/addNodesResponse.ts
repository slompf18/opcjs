 ...
 */
export class AddNodesResponse implements IEncodable {
    constructor(
    ...
    public static decode(reader: BufferReader): AddNodesResponse {
        const obj = new AddNodesResponse(
            ResponseHeader.decode(reader),
            (() => { const length = reader.readInt32(); if (length < 0) return []; const arr = new Array(length); for (let i = 0; i < length; i++) { arr[i] = AddNodesResult.decode(reader); } return arr; })(),
