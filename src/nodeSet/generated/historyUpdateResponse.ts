 ...
 */
export class HistoryUpdateResponse implements IEncodable {
    constructor(
    ...
    public static decode(reader: BufferReader): HistoryUpdateResponse {
        const obj = new HistoryUpdateResponse(
            ResponseHeader.decode(reader),
            (() => { const length = reader.readInt32(); if (length < 0) return []; const arr = new Array(length); for (let i = 0; i < length; i++) { arr[i] = HistoryUpdateResult.decode(reader); } return arr; })(),
