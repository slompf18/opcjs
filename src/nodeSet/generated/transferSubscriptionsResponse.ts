 ...
 */
export class TransferSubscriptionsResponse implements IEncodable {
    constructor(
    ...
    public static decode(reader: BufferReader): TransferSubscriptionsResponse {
        const obj = new TransferSubscriptionsResponse(
            ResponseHeader.decode(reader),
            (() => { const length = reader.readInt32(); if (length < 0) return []; const arr = new Array(length); for (let i = 0; i < length; i++) { arr[i] = TransferResult.decode(reader); } return arr; })(),
