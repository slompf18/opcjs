 ...
 */
export class CreateMonitoredItemsResponse implements IEncodable {
    constructor(
    ...
    public static decode(reader: BufferReader): CreateMonitoredItemsResponse {
        const obj = new CreateMonitoredItemsResponse(
            ResponseHeader.decode(reader),
            (() => { const length = reader.readInt32(); if (length < 0) return []; const arr = new Array(length); for (let i = 0; i < length; i++) { arr[i] = MonitoredItemCreateResult.decode(reader); } return arr; })(),
