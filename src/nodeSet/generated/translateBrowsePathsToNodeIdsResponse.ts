 ...
 */
export class TranslateBrowsePathsToNodeIdsResponse implements IEncodable {
    constructor(
    ...
    public static decode(reader: BufferReader): TranslateBrowsePathsToNodeIdsResponse {
        const obj = new TranslateBrowsePathsToNodeIdsResponse(
            ResponseHeader.decode(reader),
            (() => { const length = reader.readInt32(); if (length < 0) return []; const arr = new Array(length); for (let i = 0; i < length; i++) { arr[i] = BrowsePathResult.decode(reader); } return arr; })(),
